...
def step_fhp(current_grid, barrier, lface, rface):
    ...
    # Gap edge bounce-back: particles streaming diagonally into barrier
    ref = gap_top & n[1]; n[1] &= ~ref; n[5] |= ref  # d1->d5 at top gap edge
    ref = gap_top & n[2]; n[2] &= ~ref; n[4] |= ref  # d2->d4 at top gap edge
    ref = gap_bot & n[5]; n[5] &= ~ref; n[1] |= ref  # d5->d1 at bottom gap edge
    ref = gap_bot & n[4]; n[4] &= ~ref; n[2] |= ref  # d4->d2 at bottom gap edge
    ...
    # y=0 (top) bounce to d5 (preserve vx, reverse vy)
    ...
    # y=0 (top) bounce to d4 (preserve vx, reverse vy)
    next_grid[4, 0, :] |= n2_even[0, :]
    # x=0, y>0 (left wall) bounce to d1 (preserve vy, reverse vx)
    if len(even_rows) > 1:
    ...
    # x=0 (left wall) all even rows bounce to d5 (preserve vy, reverse vx)
    if len(even_rows) > 0:
